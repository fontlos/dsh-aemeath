// Effort FX data: effect styles, particle constants, undertow reveal state.
const EFFECT_STYLES = [
...
// Event-driven reveal arming for the undertow entry animation. The
// marker lives only while the effort panel is open: it is armed in
// commitEffort when the user really switches a tier INTO MAX (drag,
// click or keyboard), and disarmed as soon as the panel closes
// (effortOpen -> false). Reopening the panel never replays the entry
// animation because no commit happens on a plain reopen; reopening is
// not a tier change. Keyed by session so sessions never affect each
// other; survives component remounts by living at module scope.
const undertowRevealArmed = new Map();
...
export { EFFECT_STYLES, DEFAULT_STYLE, undertowRevealArmed, MATRIX, RUNNERS, SPRAY }
